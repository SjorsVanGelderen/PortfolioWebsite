...
Elm.Main.make = function (_elm) {
   ...
   var update = F2(function (action,model) {
      ...
      if (_p0.ctor === "Increase") {
            return _U.update(model,{count: model.count + _p0._0});
         } else {
            return model;
         }
   });
   ...
   var Increase = function (a) {    return {ctor: "Increase",_0: a};};
   var view = F2(function (address,model) {
      return A2($Html.div,
      _U.list([]),
      _U.list([A2($Html.div,_U.list([]),_U.list([$Markdown.toHtml(markdown)]))
              ,A2($Html.button,_U.list([A2($Html$Events.onClick,address,Increase(100))]),_U.list([$Html.text("Test button")]))]));
   });
   var Nothing = {ctor: "Nothing"};
   var mb = $Signal.mailbox(Nothing);
